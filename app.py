...
def generate_report():
    ...
    text_tempo = (
        ...
        "少し時間をかける、じっくりタイプです。\n"
        f"一方で、{female_name} さんは、その場で感じたことを\n"
        "すぐに言葉にする、テンポの速いタイプです。\n"
        "日常会話では、片方が考えている間に\n"
        "もう一方がどんどん話してしまい、\n"
        "「ちゃんと聞いてもらえていない」と\n"
        "感じる場面が出やすくなります。\n"
    ...
    text_problem = (
        f"{male_name} さんは、問題が起きたときに\n"
        "まず全体を整理してから、落ち着いて対処しようとします。\n"
        f"{female_name} さんは、感情の動きに敏感で、\n"
        "まず気持ちを共有したいタイプです。\n"
        "同じ出来事でも、片方は「どう解決するか」、\n"
        "もう片方は「どう感じたか」を大事にするため、\n"
        "タイミングがずれると、すれ違いが生まれやすくなります。\n"
        "\n"
        "一言でいうと、二人は「解決志向」と「共感志向」が、\n"
        "うまくかみ合うと心強いバランス型のペアです。"
    )

    text_values = (
        f"{male_name} さんは、安定や責任感を重視する一方で、\n"
        f"{female_name} さんは、変化やワクワク感を\n"
        "大切にする傾向があります。\n"
        "お金の使い方や休日の過ごし方、将来のイメージなど、\n"
        "小さな違いが重なると「なんでわかってくれないの？」と\n"
        "感じる瞬間が出てくるかもしれません。\n"
        "\n"
        "一言でいうと、二人の価値観は、違いを否定せずに\n"
